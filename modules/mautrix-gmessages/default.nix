...
let
  ...
  defaultConfig = {
    network = {
      displayname_template = "{{or .FullName }} (Google)";
...
in
{
  options.services.mautrix-gmessages = {
    enable = lib.mkEnableOption "mautrix-gmessages, a Matrix-GMessages puppeting bridge";
    ...
    settings = lib.mkOption {
      ...
      description = ''
        ...
        Get an example configuration by executing `mautrix-gmessages -c example.yaml --generate-example-config`
    ...
    environmentFile = lib.mkOption {
      ...
      description = ''
        ...
        If an environment variable `MAUTRIX_GMESSAGES_BRIDGE_LOGIN_SHARED_SECRET` is set,
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.mautrix-gmessages.settings.homeserver = optOneOf (
    ...
    systemd.services.mautrix-gmessages = {
      description = "Mautrix-GMessages, a Matrix-GMessages puppeting bridge";
      ...
      preStart = ''
        ...
        ${pkgs.yq}/bin/yq -s '.[0].appservice.as_token = .[1].as_token
          ...
          | .[0]
          | if env.MAUTRIX_GMESSAGES_BRIDGE_LOGIN_SHARED_SECRET then .double_puppet.secrets.[.homeserver.domain] = env.MAUTRIX_GMESSAGES_BRIDGE_LOGIN_SHARED_SECRET else . end' \
